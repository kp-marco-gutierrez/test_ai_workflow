...
def drag_card_to_column(page, card_title, target_column):
    source_card = page.locator(".card", has_text=card_title).first
    target_col = page.locator(
        ".column",
        has=page.locator(".column-header", has_text=target_column),
    )
    source_card.drag_to(target_col)
